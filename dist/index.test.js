...
import { coordinateDistance, buildGraph, buildSongFromPath, extractSubSong, songCost, isValidSong, reverseSong, concatenateSongs, singPath, dreamtimeTraversal, reachableNodes, pathExists, navigability, findCorroborees, clusterBySongs, songIntersection, findHubs, modularity, bettiNumbers, persistentCycles, persistenceBarcode, distanceMatrix, createTradition, computeFitness, mutateSong, recombineSongs, evolveGeneration, recordUsage, fittestSong, SonglineGraph, } from "./index.js";
// Helpers
const wp = (id, coords) => ({ id, coordinates: coords });
// ===== SONG MODULE =====
describe("song", () => {
    ...
    it("buildGraph creates a valid graph", () => {
        const graph = buildGraph([wp("A", [0, 0]), wp("B", [1, 0])], [{ from: "A", to: "B", weight: 1 }]);
        assert.equal(graph.nodes.size, 2);
        assert.ok(graph.edges.has("A"));
        assert.ok(graph.edges.has("B"));
    });
    it("buildSongFromPath creates a song", () => {
        const graph = buildGraph([wp("A", [0, 0]), wp("B", [1, 0]), wp("C", [2, 0])], [{ from: "A", to: "B", weight: 1 }, { from: "B", to: "C", weight: 2 }]);
    ...
    it("extractSubSong extracts a range", () => {
        const graph = buildGraph([wp("A", [0, 0]), wp("B", [1, 0]), wp("C", [2, 0]), wp("D", [3, 0])], [
    ...
    it("songCost sums verse weights", () => {
        const graph = buildGraph([wp("A", [0, 0]), wp("B", [1, 0]), wp("C", [2, 0])], [{ from: "A", to: "B", weight: 3 }, { from: "B", to: "C", weight: 7 }]);
    ...
    it("isValidSong detects invalid songs", () => {
        ...
        const bad = { id: "x", waypoints: [wp("A", [0]), wp("B", [1])], verses: [] };
        assert.ok(!isValidSong(bad));
    });
    it("reverseSong reverses the song", () => {
        const graph = buildGraph([wp("A", [0]), wp("B", [1]), wp("C", [2])], [{ from: "A", to: "B", weight: 1 }, { from: "B", to: "C", weight: 2 }]);
        const song = buildSongFromPath(graph, ["A", "B", "C"]);
...
describe("navigation", () => {
    function makeTriangleGraph() {
        return buildGraph([wp("A", [0, 0]), wp("B", [4, 0]), wp("C", [2, 3])], [
            { from: "A", to: "B", weight: 1 },
    ...
    it("singPath uses dreamtime for disconnected graph", () => {
        const graph = buildGraph([wp("A", [0, 0]), wp("B", [10, 0])], []);
    ...
    it("dreamtimeTraversal always finds a path", () => {
        const graph = buildGraph([wp("A", [0, 0]), wp("B", [5, 5]), wp("C", [10, 10])], []);
    ...
    it("reachableNodes finds all reachable nodes", () => {
        const graph = buildGraph([wp("A", [0]), wp("B", [1]), wp("C", [2])], [{ from: "A", to: "B", weight: 1 }]);
    ...
    it("navigability computes correctly", () => {
        const graph = buildGraph([wp("A", [0]), wp("B", [1]), wp("C", [2])], [
            { from: "A", to: "B", weight: 1 },
            { from: "B", to: "C", weight: 1 },
        ]);
    ...
    it("navigability is 1 for fully connected graph", () => {
        const graph = buildGraph([wp("A", [0]), wp("B", [1])], [{ from: "A", to: "B", weight: 1 }]);
...
describe("corroboree", () => {
    function makeSongs() {
        const graph = buildGraph([wp("A", [0, 0]), wp("B", [1, 0]), wp("C", [2, 0]), wp("D", [0, 1])], [
            { from: "A", to: "B", weight: 1 },
            { from: "B", to: "C", weight: 1 },
