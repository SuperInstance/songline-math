 ...
 */
export function findCorroborees(songs) {
    const waypointSongs = new Map();
    const waypointData = new Map();
    const waypointWeights = new Map();
 ...
 */
export function clusterBySongs(songs, threshold = 2) {
    // Build: nodeId → set of song IDs
    const nodeSongs = new Map();
    for (const song of songs) {
        for (const wp of song.waypoints) {
            if (!nodeSongs.has(wp.id))
                nodeSongs.set(wp.id, new Set());
 ...
 */
export function songIntersection(a, b) {
 ...
 */
export function findHubs(graph, topN = 5) {
 ...
 */
export function modularity(graph, clusters) {
    let totalEdges = 0;
...
//# sourceMappingURL=corroboree.js.map