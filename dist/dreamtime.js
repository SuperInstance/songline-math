 ...
 */
export function distanceMatrix(graph) {
 ...
 */
export function bettiNumbers(graph, threshold) {
 ...
 */
export function persistentCycles(graph, numSteps = 20) {
    const { ids, distances } = distanceMatrix(graph);
    const n = ids.length;
    if (n < 3)
        return [];
 ...
 */
function bettiNumbersAtThreshold(_ids, distances, n, threshold) {
    const parent = new Array(n);
    for (let i = 0; i < n; i++)
        parent[i] = i;
    const find = (x) => {
 ...
 */
function findCycleAtThreshold(ids, distances, n, threshold) {
 ...
 */
export function persistenceBarcode(graph, numSteps = 20) {
    const { ids, distances } = distanceMatrix(graph);
    const n = ids.length;
    let maxDist = 0;
    for (let i = 0; i < n; i++) {
...
//# sourceMappingURL=dreamtime.js.map