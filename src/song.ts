 ...
 */
export function coordinateDistance(a: Coordinates, b: Coordinates): number {
 ...
 */
export function buildGraph(waypoints: Waypoint[], verses: Verse[]): KnowledgeGraph {
  const nodes = new Map<string, Waypoint>();
  const edges = new Map<string, Verse[]>();

  for (const wp of waypoints) {
    nodes.set(wp.id, wp);
    if (!edges.has(wp.id)) edges.set(wp.id, []);
  }

  for (const v of verses) {
    if (!edges.has(v.from)) edges.set(v.from, []);
    edges.get(v.from)!.push(v);
    // Also store reverse edge for undirected traversal
    if (!edges.has(v.to)) edges.set(v.to, []);
 ...
 */
export function buildSongFromPath(
  ...
  for (const id of path) {
    ...
    if (!wp) throw new Error(`Waypoint not found: ${id}`);
    waypoints.push(wp);
  }
  ...
  for (let i = 0; i < path.length - 1; i++) {
    ...
    if (!edge) throw new Error(`No verse from ${from} to ${to}`);
 ...
 * @returns A sub-song
 */
export function extractSubSong(song: Song, startIdx: number, endIdx: number): Song {
 ...
 * @returns Total cost
 */
export function songCost(song: Song): number {
  let total = 0;
 ...
 */
export function isValidSong(song: Song): boolean {
  if (song.waypoints.length === 0) return true;
  if (song.verses.length !== song.waypoints.length - 1) return false;
 ...
 */
export function reverseSong(song: Song): Song {
  return {
    id: `${song.id}-reverse`,
 ...
 */
export function concatenateSongs(a: Song, b: Song): Song {
  if (a.waypoints.length === 0) return { ...b, id: `${a.id}+${b.id}` };
  if (b.waypoints.length === 0) return { ...a, id: `${a.id}+${b.id}` };
